...
if [ $RABBITMQ_YN = y ]; then
  ...
  [[ "$(rpm -qa | grep erlang)" ]] || yum install erlang-18.2-1.el6

  [[ "$(rpm -qa | grep rabbitmq-server)" ]] || rpm --import http://www.rabbitmq.com/rabbitmq-signing-key-public.asc
  [[ "$(rpm -qa | grep rabbitmq-server)" ]] || wget http://www.rabbitmq.com/releases/rabbitmq-server/v3.6.0/rabbitmq-server-3.6.0-1.noarch.rpm
  [[ "$(rpm -qa | grep rabbitmq-server)" ]] || yum install rabbitmq-server-3.6.0-1.noarch.rpm -y 
...
if [ $BACKEND_YN = y ]; then
  WGET_BACK_CK="cloudike-backend-clx-0.1-1438004096.el6.x86_64.rpm"
  ...
  [[ -f $WGET_BACK_CK ]] || wget ${REPO_URL}cloudike-backend-clx-0.1-1438004096.el6.x86_64.rpm
  ...
  INSTALL_BACK_CK="cloudike-backend-clx-0.1-1438004096.el6.x86_64"
  ...
  [[ "$(rpm -qa | grep $INSTALL_BACK_CK)" ]] || rpm -Uvh cloudike-backend-clx-0.1-1438004096.el6.x86_64.rpm
...
# mountbit.backend가 아니라 clx.backend 로 변경되었음(최신버전에서만 적용해야함) - 20160115.
if [ $WORKER_YN = y ]; then
  echo '[program:celery]
