...
bind_ip=$HOSTIP ###!!!
configsvr=true" > /etc/mongod.conf
...
pidfilepath=/var/run/mongodb/mongod.pid
bind_ip=$HOSTIP ###!!!
replSet=shard1" > /etc/mongod.conf
      ...
      if [ $MONGOS_YN = y ]; then
         echo 'rs.initiate()' | mongo $MONGODB_MASTER_IP:27017 && sleep 10
	       for value in "${MONGODB_CLUSTER_LIST[@]}"; do
             echo "rs.add( '$value:27017' )" | mongo $MONGODB_MASTER_IP:27017
         done
	 
	       echo "*** After mongo, you must execute follow command ***"
         echo "cfg = rs.conf();"
	       echo "cfg.members[0].host = \"$MONGODB_MASTER_IP:27017\"";
         echo "rs.reconfig(cfg);"
         echo "exit;"
         mongo $MONGODB_MASTER_IP:27017
...
start() {
    ...
    daemon --user mongod "$exec --quiet $OPTIONS >> $logfile 2>&1 &"
    retval=$?
    echo
...
' > /etc/init.d/mongos
         ...
         echo OPTIONS=\'-f /etc/mongos.conf --setParameter "releaseConnectionsAfterResponse=true"\' > /etc/sysconfig/mongos
         ...
         service mongos start
	       ...
	       echo "*** After mongo, you must execute follow command ***"
         echo "sh.addShard(\"shard1/MONGODB_MASTER:27017\");"
         echo "exit;"
         mongo
      fi
...
if [ $RABBITMQ_YN = y ]; then
  ...
  if [ $MULTI_YN = y ]; then
    ...
    if [ $MONGODB_MASTER_YN = y ]; then
    
      for value in "${MONGODB_CLUSTER_LIST[@]}"; do
        scp /var/lib/rabbitmq/.erlang.cookie $value:/var/lib/rabbitmq/.erlang.cookie
        ssh $value 'chown rabbitmq:rabbitmq /var/lib/rabbitmq/.erlang.cookie'
...
if [ $BACKEND_YN = y ]; then
  ...
  [[ "$(rpm -qa | grep $INSTALL_BACK_CK)" ]] || rpm -Uvh cloudike-backend-clx-0.1-1439572570.el6.x86_64.rpm
